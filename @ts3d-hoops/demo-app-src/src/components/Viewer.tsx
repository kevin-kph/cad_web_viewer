...
import { getService, LogEntry, LogLevel } from '@ts3d-hoops/web-viewer-components';
...
export function formatLogLevelName(level: LogLevel): TypeOptions {
  switch (level) {
    case 'info':
      return 'info';
    case 'warn':
...
export function showToast(entry: LogEntry) {
  const type = formatLogLevelName(entry.level);
  const message = entry.context?.['details']
    ? `${entry.message}\n${String(entry.context['details'])}`
    : entry.message;

  toast(message, {
    type,
    autoClose: entry.level === 'error' ? false : 5000,
  });
}
...
export function Viewer(props: ViewerProps) {
  ...
  const loggerInitialized = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    const logger = getService('LogService');
    logger.addEventListener('hoops-log-service-entry', (e) => {
      const logEvent = e as CustomEvent<LogEntry>;
      showToast(logEvent.detail);
    });
